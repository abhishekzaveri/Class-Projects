	...
	#include "familyRunner.h"
         ...
         */
        template <class HashedObj>
        int QuadraticHashTable<HashedObj>::insert( const HashedObj & x, int familyCount )
        {
...
/* 2*/      int currentPos = hash( x, familyCount );

/* 3*/      while( array[ currentPos ].info != EMPTY &&
                   array[ currentPos ].element.person.firstName != x.person.firstName && array[ currentPos ].element.person.lastName != x.person.lastName &&
		   array[ currentPos ].element.person.year != x.person.year )
            {
...
/*	template <class HashedObj>
	bool QuadraticHashTable<HashedObj> & QuadraticHashTable<HashedObj>::
	 operator!=( const QuadraticHashTable<HashedObj> & rhs)
	{
		return !(this == rhs);
		//return !(this.person.firstName == rhs.person.firstName && this.person.lastName == rhs.person.lastName && this.year == rhs.year)
	}
*/
        /**
	    ...
	    tableSize *= 2;
            for( int i = 0; i < 4 ; i++)
                hashVal += (int)key.person.firstName[i]+ (int)key.person.lastName[i];
	    hashVal+= key.person.year;
